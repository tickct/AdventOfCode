...
const dataArray= toDataFormatted(path.join('.','2021','Problem15','Input.txt'),
  splitLine,
  splitAll,
  toLargeBoard,
  (rows) => rows.map((row, r) => row.map((val,c) => ({value:parseInt(val), visit: false, distance:( r === 0 && c === 0 )? 0 : Number.MAX_SAFE_INTEGER})))
);
...
const updateBoard = ([r,c], board) => {
  return [
    ...board.slice(0,r),
    board[r].map((node,i) => i === c ? {value: node.value, visit: true, distance: node.distance} : node),
    ...board.slice(r+1)
  ]
}
...
const updateDistance = (cord,board) => {
  ...
  const neighborNodes = [getLeft,getRight,getUp,getDown].map(fn => fn(cord)).filter(x => !isOutOfBounds(x));
  const newBoard = updateBoard(cord, board)
  return neighborNodes.reduce((upBoard, [r,c]) => {
    return [
      ...upBoard.slice(0,r),
      upBoard[r].map((node,i) => i === c ? {value: node.value, visit: true, distance: Math.min(node.value+currentNode.distance, node.distance)} : node),
      ...upBoard.slice(r+1)
    ]
  },newBoard)
}
...
console.log(dataArray
  .reduce((board,row, ri) => 
    row.reduce(
      (board2, col, ci) => updateDistance([ri,ci], board2),
      board
    )
    ,dataArray
  )[MAXROW][MAXCOL])