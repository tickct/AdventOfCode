...
const dataArray = toDataFormatted(
  path.join('.', '2021', 'Problem12', 'input.txt'),
  splitLine,
...
const clearPaths = (location, pathObj) =>
...
const findPaths = (links, path) => {
  ...
  return possibleNext.reduce(
    (count, nextNode) => count + findPaths(newLinks, [nextNode, ...path]),
    0
  )
}
...
const findPaths2 = (links, path, hasUsedDouble) => {
  const currentNode = path[0]
  if (currentNode === 'end') return 1
  const revisitedSmall =
    path.filter((p) => p === currentNode.toLowerCase()).length >= 2
  const clearedDupLinks =
    revisitedSmall && !hasUsedDouble
      ? path
          .filter((x) => x === x.toLowerCase())
          .reduce((link, nextChar) => clearPaths(nextChar, link), links)
      : links
  const newLinks =
    ((revisitedSmall || hasUsedDouble) &&
      currentNode !== currentNode.toUpperCase()) ||
    ['start', 'end'].includes(currentNode)
