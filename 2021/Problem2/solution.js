...
const toInstructions = arr => arr.map(val => {
  const [instruction, amount] = val.split(' ');
  return {instruction, amount: parseInt(amount)};
})
const dataArray= toDataFormatted(path.join('.','2021','Problem2','input.txt'), splitLine, toInstructions)
...
const toMoveFunctions = arr => {
  return arr.map(({instruction, amount}) => instructionMap[instruction](amount))
}

const finalCord = toMoveFunctions(dataArray).reduce((pos, moveFn) => {
  return moveFn(pos)
}, [0,0,0]
)
