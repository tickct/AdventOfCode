...
const toStacks = stackStr => {
  ...
  const lineups = arrs.map(line => _.chunk(line,4)).map(line => line.map(val => val[1]))
...
console.log(calcInstruction.reduce((currStack, instruction) => 
 {
  return move(instruction,currStack)
 },foo).map(stack => stack.pop()).join(''))
