...
const dataArray= toDataFormatted(path.join('.','2021','Problem6','input.txt'),
  splitComma,
  strValArr => strValArr.map(strVal => parseInt(strVal)) 
)

const cycleLife = fishArr => fishArr.map(lifeCount => lifeCount === 0 ? 6 : lifeCount-1)
...
const toInitialFishDic = fishArr => fishArr.reduce((fishDic, age) => ({...fishDic, [age]: (fishDic[age] || 0) + 1}), {})
const countFish = fishObj => Object.values(fishObj).reduce((count, next) => count+next, 0);
