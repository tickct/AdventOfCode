...
const isVisibleRight = (arr) =>
  arr.reduce((vis, next, i) => {
    if (next <= Math.max(...arr.slice(0, i))) return [...vis, false]
    return [...vis, true]
  }, [])

const isVisibleLeft = (arr) =>
  arr.reduce((vis, next, i) => {
    if (next <= Math.max(...arr.slice(i + 1))) return [...vis, false]
    return [...vis, true]
  }, [])

const isVisableTop = (twoArr) =>
  twoArr.map((arr, twoI) =>
    arr.reduce((vis, next, i) => {
      if (next <= Math.max(...twoArr.slice(0, twoI).map((arr) => arr[i])))
        return [...vis, false]
      return [...vis, true]
    }, [])
  )

const isVisableBottom = (twoArr) =>
  twoArr.map((arr, twoI) =>
    arr.reduce((vis, next, i) => {
      if (next <= Math.max(...twoArr.slice(twoI + 1).map((arr) => arr[i])))
        return [...vis, false]
      return [...vis, true]
    }, [])
  )
...
const toVis = (...visTwoArrs) =>
  visTwoArrs[0].map((line, i) => _.zip(...visTwoArrs.map((arr) => arr[i])))

const visible = toVis(visRight, visLeft, visTop, visBottom).map((line) =>
  line.map((val) => val.some((x) => x))
)

console.log(sum(visible.map((row) => row.filter((x) => x).length)))

const toScoreRight = (arr) =>
  arr.map((next, i) => {
...
const toScoreTop = (twoArr) =>
  twoArr.map((arr, twoI) =>
    arr.map((next, i) => {
      const treesLeftOrdered = twoArr
        .slice(0, twoI)
        .map((arr) => arr[i])
        .reverse()
      const tallerTree = treesLeftOrdered.findIndex((tree) => tree >= next)
...
const toScores = (...scoreTwoArrs) =>
  scoreTwoArrs[0].map((line, i) => _.zip(...scoreTwoArrs.map((arr) => arr[i])))
const scores = toScores(scoreRight, scoreLeft, scoreTop, scoreBottom).map(
  (row) => row.map(([a, b, c, d]) => a * b * c * d)
)
