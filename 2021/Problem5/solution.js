...
const dataArray = toDataFormatted(
  path.join('.', '2021', 'Problem5', 'input.txt'),
  splitLine,
  (lines) => lines.map((line) => line.split('->').map((x) => x.trim())),
  (lines) =>
    lines.map((deltaStringArr) =>
      deltaStringArr.map((coordString) =>
        coordString.split(',').map((x) => parseInt(x))
      )
    )
)

const dataArrayNoDiag = dataArray.map((lines) =>
  lines.filter(
    (deltaStringArr) =>
      deltaStringArr[0][0] === deltaStringArr[1][0] ||
      deltaStringArr[0][1] === deltaStringArr[1][1]
  )
)
...
const filledDictionary = dataArray.reduce((ventDic, [[x1, y1], [x2, y2]]) => {
  ...
  return incFrom(
    ...
    ([x, y]) => [x + Math.sign(x2 - x1), y + Math.sign(y2 - y1)]
  )
}, {})
