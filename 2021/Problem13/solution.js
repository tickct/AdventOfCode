...
const dataArray = toDataFormatted(
  path.join('.', '2021', 'Problem13', 'Input.txt'),
  splitLine
)
...
const instructions = dataArray
  .filter((row) => row.includes('fold'))
  .map((x) => x.match(foldREGEX))
  .map((x) => [x[1], parseInt(x[2])])
const toDimensions = (points) =>
  points.reduce(
    ([x, y], [x2, y2]) => [Math.max(x, x2), Math.max(y, y2)],
    [0, 0]
  )

const makeGrid = ([c, r]) =>
  Array.from(new Array(r + 1), (x) => Array(c + 1).fill(''))
...
const foldX = (grid, val) => {
  ...
  const combined = halfNoMove.map((row, i) =>
    _.zip(row, halfMove[i]).map(([a, b]) => a || b || 0)
  )
  return combined
}
...
console.log(
  instructions.reduce(
    (nextGrid, instruction) => fold(nextGrid, instruction),
    grid
  )
)
