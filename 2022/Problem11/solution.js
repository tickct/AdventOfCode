...
const doCycle = (monkeyArr, lcmNumber) =>
  monkeyArr.reduce((monkeyCurrArr, currMonkey, i) => {
    ...
    items.forEach((num) => {
      ...
      newMonkeyArr[nextIndex] = addItem(
        newMonkeyArr[nextIndex],
        examineWorry % lcmNumber
      )
    })
    return newMonkeyArr
  }, monkeyArr)
...
const cycleN = (n, curr) => {
  return Array(n)
    .fill(' ')
    .reduce((rCurr) => doCycle(rCurr, lcmNumber), curr)
}

const final = cycleN(10000, monkeys)

const finalCounts = final
  .map((moneky) => moneky.inspected)
  .sort((a, b) => b - a)
