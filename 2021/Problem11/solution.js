...
const dataArray= toDataFormatted(path.join('.','2021','Problem11','input.txt'),
  splitLine,
  splitAll,
  (cord2D) => cord2D.map(row => row.map(val => ({value: parseInt(val), visit: false})))
);
...
const flashChain = (cord2D, count = 0) => {
  const flashRowIndex = cord2D.findIndex((row) => row.find(({value, visit}) => value > 9 && !visit))
  if(flashRowIndex === -1) return [cord2D,count];
  const flashColIndex = cord2D[flashRowIndex].findIndex(({value, visit}) => value > 9 && !visit)
...
const doNStep = (cord2D, n, count = 0) => {
  ...
  const resetCord = flashCord2D.map(row => row.map(({value, visit}) => ({ value: value >  9 ? 0 : value, visit:false})))
...
const findSync = (cord2D, n = 1) => {
  ...
  const [flashCord2D,flashCount] = flashChain(incCord2D,0);
  const resetCord = flashCord2D.map(row => row.map(({value, visit}) => ({ value: value >  9 ? 0 : value, visit:false})))
  return flashCount === size
    ? n
    : findSync(resetCord, n+1)
}
