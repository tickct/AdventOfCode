...
const dataStream = toDataFormatted(
  ...
  splitLine,
  lines => lines.map(str => str.split('').map(x => {
    if(x === 'S') { return START_VAL }
    if(x === 'E') { return END_VAL}
...
const canMoveMap = (grid, cord) => {
  const oneStepMax = (cord, checkFn) => toGridValue(grid,cord) - toGridValue(grid,checkFn(cord)) >= -1
  const chordIf = (cord, checkFn) => oneStepMax(cord, checkFn) ? checkFn(cord) : null
  return [chordIf(cord, nodeLeft),chordIf(cord, nodeRight),chordIf(cord, nodeUp),chordIf(cord, nodeDown)].filter(x => x)
}

const toNodes = gridNum => gridNum.map((row, r) => row.map((val, c) => ({
  visit: false,
...
const findChordVal = (searchMap, target) => {
  const r = searchMap.findIndex(nodeRow => nodeRow.find(node => node.val === target))
...
const starts = dataStream.reduce((starts, nextRow, r) => [...starts, ...nextRow.map((cell, c) => cell <= 97
  ? [r,c]
  : undefined
).filter(x => x)],[]);
...
const findPaths = (grid, cord) => {
  ...
  while(queue.length){
    ...
    const toAddToQueue = nextNode.adjacent.filter(cord => 
