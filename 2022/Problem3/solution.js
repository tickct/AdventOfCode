...
const getPounches = (lines) => lines.map((line) => _.chunk(line,line.length/2))

const findPair = ([a,b]) =>  _.intersection(a,b)

const getNumber = x => x.charCodeAt(x)

const subToVal = x => x === x.toUpperCase() ? getNumber(x)-38 : getNumber(x)-96

console.log(sum(getPounches(dataArray).map(pair =>subToVal((findPair(pair)[0])))))

const getGroups = dataArr => _.chunk(dataArr.map(data => data.split('')),3)
