...
const partNumbers = fullNums
  .map((row, r) => {
    const partNumbers = row.map((val, c) =>
      isPartNumber([r, c]) && /\d/.test(val) ? val : undefined
    )
    return partNumbers
      .filter((x, i): x is string => {
    ...
    return row
      .map((item, c) => {
        if (/\*/.test(item) && nextToNumber([r, c])) {
          return [
            ...new Set(toGetNeighbors(fullNums)([r, c]).filter(isNumberString)),
          ]
        }
        return undefined
      })
