...
const dataArray= toDataFormatted(path.join('.','2021','Problem14','Input.txt'),
  splitLine,
);

const start = dataArray[0];

const instructions = dataArray.slice(1).filter(x => x).reduce((instructionObj, next) => {
...
const doCycle = (input, n) => {
  if(n === 0) return input
  const update = input.split('').map((letter,i) => {
...
const counts = (input) => {
  ...
  const counts = uniqe.reduce((counts, next) => ([
    ...counts,
...
const toCountObj = input => input.split('').reduce((countObj, next, i, arr) => {
  if(arr.length === i +1) {
    return {
      ...countObj,
    }
...
const countsV2 = (countObj,start) => Object.values(
  Object.entries(countObj)
    .reduce((singleCounts, [pair, count]) => {
      return pair.split('').reduce((runningCount, next) => ({...runningCount, [next]: (runningCount[next] || 0) + count}),singleCounts)
    }, {
      [toFirst(start)]: 1,
      [toLast(start)]: 1
    })
