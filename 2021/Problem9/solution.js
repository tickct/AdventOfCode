...
const dataArray= toDataFormatted(path.join('.','2021','Problem9','input.txt'),
  splitLine,
  splitAll
)
...
const checkNeigbor = moveFn => ([or,oc]) => {
  const [cr,cc] = moveFn([or,oc]);
  if(isOutOfBounds([cr,cc])) { return true };
...
const getRisk = lowArr => sum(lowArr.map(row => 
...
console.log(getRisk(dataArray.map((row,rowI) => row.filter((col, colI) => checkAll([rowI,colI])))))