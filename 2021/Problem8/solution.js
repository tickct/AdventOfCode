...
const dataArray = toDataFormatted(
  path.join('.', '2021', 'Problem8', 'input.txt'),
  splitLine,
...
const countUnique = (digArrs) => {
  return digArrs.reduce((count, digArr) => {
    ...
    return (
      digArr.filter((x) => valid.some((y) => y === x.length)).length + count
    )
  }, 0)
}
...
const solidify = (possibleObj) => {
  return Object.keys(possibleObj).reduce((solidObj, number) => {
    return {
      ...solidObj,
      [number]:
        possibleObj[number].length === 1
          ? possibleObj[number][0]
          : possibleObj[number],
    }
  }, {})
}
...
const removeKnown = (keyOptions, key) =>
  keyOptions.filter((option) => _.difference(option, key).length !== 0)

const find0 = (possibleObj) => {
  return {
    ...possibleObj,
    0: possibleObj[0].find(
      (x) => !containsAll(_.difference(possibleObj[4], possibleObj[1]))(x)
    ),
  }
}
...
const find9 = (possibleObj) => {
  return {
    ...possibleObj,
    6: removeKnown(
      removeKnown(possibleObj[6], possibleObj[0]),
      possibleObj[9]
    )[0],
  }
}
...
const find5 = (possibleObj) => {
  return {
    ...possibleObj,
    5: possibleObj[5].find((x) =>
      x.every((y) => possibleObj[6].find((z) => z === y))
    ),
  }
}
...
const find2 = (possibleObj) => {
  return {
    ...possibleObj,
    2: removeKnown(
      removeKnown(possibleObj[2], possibleObj[5]),
      possibleObj[3]
    )[0],
  }
}
...
const solve = (fourDigitArr, key) =>
  fourDigitArr.map((val) => {
