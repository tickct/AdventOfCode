...
const dataArray= toDataFormatted(path.join('.','2021','Problem3','input.txt'), splitLine, (valArr) => valArr.map(x => x.split('')))
...
const valueRead = dataArray[0]
  ...
  .reduce(([mostCurr, leastCurr], next) => [mostCurr+next[0],leastCurr+next[1]],['','']);
...
const toRating = (dataArr, compareFn, count = 0) => {
  ...
  return compareFn(ones.length, zeros.length) ? toRating(ones,compareFn, count+1): toRating(zeros, compareFn, count+1);
}
